...
Higher within-subject variability **reduces** power because:
1. **Noise obscures signal**: More day-to-day variation makes it harder to detect true intervention effects
2. **Measurement precision**: Greater variability around each person's "true" behavior level adds noise
3. **Effect detectability**: Small but real intervention effects can be masked by high natural variation

This is why the power analysis shows that studies with participants who have very consistent routines (low SD) are more likely to detect intervention effects than studies with highly variable participants.
