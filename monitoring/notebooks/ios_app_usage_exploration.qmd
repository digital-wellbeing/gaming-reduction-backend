...
if (REFRESH_DATA && (needs_main_refresh || needs_contact_refresh)) {
  ...
  pipeline_result <- tryCatch({
    cat("⏱️ This may take several minutes to download screenshots, run OCR analysis, and pull Qualtrics data...\n")
    system2(
      "python3",
      args = c("../../monitoring/join_diary_ios.py", "--output-dir", "../../.tmp", "--verbose"),
      stdout = TRUE,
      stderr = TRUE,
      wait = TRUE,
      timeout = 3600  # 60 minutes timeout for full iOS pipeline
    )
  }, error = function(e) {
    cat("❌ Error in iOS pipeline:", e$message, "\n")
    return(NULL)
  })
  
  if (!is.null(attr(pipeline_result, "status")) && attr(pipeline_result, "status") != 0) {
    cat("⚠️ Warning: iOS pipeline exited with non-zero status:", attr(pipeline_result, "status"), "\n")
    if (length(pipeline_result) > 0) {
      cat("Last 20 lines of output:\n")
      cat(paste(tail(pipeline_result, 20), collapse = "\n"), "\n\n")
    }
  } else if (!is.null(pipeline_result)) {
    cat("✅ iOS pipeline completed successfully\n")
    if (length(pipeline_result) > 5) {
      cat("Last few lines of output:\n")
      cat(paste(tail(pipeline_result, 5), collapse = "\n"), "\n\n")
    }
  } else {
    cat("❌ iOS pipeline failed or timed out\n")
  }
  
  cat("🎉 iOS data pipeline completed!\n\n")
...
gaming_apps_summary <- ios_processed %>%
  ...
  group_by(App) %>%
  summarise(
    participant_count = n_distinct(PID),
    total_sessions = n(),
    total_duration = sum(Duration, na.rm = TRUE),
...
datatable(gaming_apps_summary,
          ...
          filter = 'top') %>%
          formatRound(columns = c(3, 5, 6, 7), digits = 1)
