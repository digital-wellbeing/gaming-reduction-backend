...
# WARNING: Data refresh can take 10+ minutes due to image downloads and OCR processing
# NOTE: OCR analysis now skips already-analyzed images for efficiency
# To force reprocessing of all images, you'll need to manually run with --reprocess-existing
REFRESH_DATA <- TRUE
...
FINAL_OUTPUT_FILE <- "../../downloads/ios/aggregated_participant_data_enriched.csv"
...
if (REFRESH_DATA && (needs_main_refresh || needs_contact_refresh)) {
  ...
  # Step 1: Pull contact list data from Qualtrics (FIRST - other steps depend on this)
  cat("Step 1/6: Pulling contact list data from Qualtrics...\n")
  step1_result <- tryCatch({
    system2(
      "python3",
      args = c("../pull_contact_list.py", "--output", "contact_list_with_embedded"),
      stdout = TRUE,
      stderr = TRUE,
      wait = TRUE
    )
  ...
  if (!is.null(attr(step1_result, "status")) && attr(step1_result, "status") != 0) {
    cat("⚠️ Warning: Step 1 exited with non-zero status\n")
  ...
  # Step 2: Download Qualtrics images
  ...
  step2_result <- tryCatch({
    system2(
      "python3",
      args = c("../../monitoring/qualtrics_image_downloader.py", "--verbose"),
      stdout = TRUE,
      stderr = TRUE,
      wait = TRUE,
  ...
  if (!is.null(attr(step2_result, "status")) && attr(step2_result, "status") != 0) {
    cat("⚠️ Warning: Step 2 exited with non-zero status\n")
  ...
  # Step 3: OCR analysis with Gemini (process all iOS images)
  ...
  if (!is.null(attr(step3_result, "status")) && attr(step3_result, "status") != 0) {
    cat("⚠️ Warning: Step 3 exited with non-zero status\n")
  ...
  # Step 4: Aggregate participant data for all participants
  ...
  step4_result <- tryCatch({
    system2(
      ...
      args = c("../../monitoring/ocr/participant_aggregator.py",
               "--group",
               "--base-dir", "../../downloads/ios",
               "--qualtrics-csv", "../../downloads/HFF Gaming Reduction 3 - Daily Survey.csv",
               "--output-report", "../../downloads/ios/participant_summary.json"),
      stdout = TRUE,
      stderr = TRUE,
      wait = TRUE
    )
  ...
  if (!is.null(attr(step4_result, "status")) && attr(step4_result, "status") != 0) {
    cat("⚠️ Warning: Step 4 exited with non-zero status\n")
  ...
  # Step 5: Convert summary JSON to CSV
  cat("\nStep 5/6: Converting summary to CSV...\n")
  step5_result <- tryCatch({
    system2(
      "python3",
      args = c("../../monitoring/ocr/summary_to_csv.py",
               "../../downloads/ios/participant_summary.json",
               "--output", "../../downloads/ios/aggregated_participant_data.csv"),
      stdout = TRUE,
      stderr = TRUE,
      wait = TRUE
    )
  ...
  if (!is.null(attr(step5_result, "status")) && attr(step5_result, "status") != 0) {
    cat("⚠️ Warning: Step 5 exited with non-zero status\n")
  ...
  # Step 6: Classify gaming apps
  cat("\nStep 6/6: Classifying gaming applications...\n")
  step6_result <- tryCatch({
    system2(
      "python3",
      args = c("../../monitoring/ocr/app_game_classifier.py",
               "../../downloads/ios/aggregated_participant_data.csv",
               "--output", "../../downloads/ios/aggregated_participant_data_enriched.csv",
               "--format", "ios"),
      stdout = TRUE,
      stderr = TRUE,
      wait = TRUE
  ...
  if (!is.null(attr(step6_result, "status")) && attr(step6_result, "status") != 0) {
    cat("⚠️ Warning: Step 6 exited with non-zero status\n")
...
for (file in files_to_check) {
  if (file.exists(file)) {
    ...
    cat(sprintf("✗ %s not found - data pipeline may have failed\n", basename(file)))
  }
}
cat("\n")
```
...
ios_data <- read_csv("../../downloads/ios/aggregated_participant_data_enriched.csv")

# Load iOS-specific participant report if available
if (file.exists("../../.tmp/participant_report_ios.csv")) {
  participant_report_ios <- read_csv("../../.tmp/participant_report_ios.csv")
  cat("✓ Loaded iOS-specific participant report with", nrow(participant_report_ios), "participants\n")
  
  # Show iOS participants summary
  ios_with_data <- sum(participant_report_ios$has_ios_data, na.rm = TRUE)
  cat("  Participants with iOS platforms:", ios_with_data, "\n")
} else {
  cat("⚠️ iOS-specific participant report not found. Run join_diary_ios.py to generate it.\n")
...
if (file.exists("../../.tmp/contact_list_with_embedded.csv")) {
  ...
  if (nrow(participants_without_condition) > 0) {
    cat("⚠️ WARNING: Found", nrow(participants_without_condition), "participants without contact list data:\n")
  ...
  cat("⚠️ WARNING: Contact list file not found. Using fallback from daily survey file.\n")
  ...
  enrollment_data <- read_csv("../../downloads/HFF Gaming Reduction 3 - Daily Survey.csv") %>%
...
# Debug: Check ios_processed data structure first
cat("Debugging ios_processed data:\n")
cat("- Number of rows:", nrow(ios_processed), "\n")
if (nrow(ios_processed) > 0) {
  cat("- Column names:", paste(names(ios_processed), collapse = ", "), "\n")
  cat("- Unique DeviceTypes:", paste(unique(ios_processed$DeviceType), collapse = ", "), "\n")
  cat("- Study day range:", range(ios_processed$study_day, na.rm = TRUE), "\n")
  cat("- Number of unique participants:", n_distinct(ios_processed$PID), "\n")
} else {
  cat("❌ ERROR: ios_processed is empty! Check data loading and processing steps.\n")
  # Create empty objects to prevent further errors
  complete_grid <- data.frame()
  availability_summary_by_device <- data.frame()
...
if (nrow(ios_processed) > 0) {
  ...
  cat("\nGaming duration check by device type:\n")
  ...
  if (nrow(complete_grid) > 0) {
    ...
    cat("Column names in complete_grid:", paste(names(complete_grid), collapse = ", "), "\n")
  }

  # Only create summaries if we have data in complete_grid
  if (nrow(complete_grid) > 0 && "DeviceType" %in% names(complete_grid) && "study_day" %in% names(complete_grid)) {
    # Summary statistics by device type
    availability_summary_by_device <- complete_grid %>%
      group_by(DeviceType, study_day) %>%
      summarise(
        participants_with_data = sum(has_data, na.rm = TRUE),
    ...
    cat("❌ ERROR: complete_grid is missing required columns or is empty.\n")
    # Create empty summaries to prevent further errors
    availability_summary_by_device <- data.frame()
    availability_summary <- data.frame()
  }
} else {
  cat("Skipping data availability matrix due to empty ios_processed data.\n")
}
```
...
if (exists("complete_grid") && nrow(complete_grid) > 0 && exists("device_types") && length(device_types) > 0) {
  ...
  if(nrow(device_data) > 0) {
    ...
    device_heatmap <- device_data %>%
      ...
      mutate(
        participant_label = paste0(PID, " (", str_extract(study_completion_status, "^[🟡✅🔴❓]"), ")")
      ) %>%
...
print(combined_heatmap)
  ...
  cat("No data available for heatmap visualization.\n")
}
```
...
```{r gaming-timeseries}
# Only proceed if we have complete_grid data
if (exists("complete_grid") && nrow(complete_grid) > 0) {
...
if(total_gaming_time > 0) {
  ...
  cat("No complete_grid data available. Skipping gaming time series analysis.\n")
}
```
...
# Only proceed if we have ios_processed data
if (exists("ios_processed") && nrow(ios_processed) > 0) {
...
if (exists("top_gaming_by_participant") && nrow(top_gaming_by_participant) > 0) {
  ...
  if (nrow(sample_participants) > 0) {
    ...
    for(i in 1:min(nrow(sample_participants), 50)) {  # Show first 50 entries
      ...
      if (!is.na(row$PID) && row$PID != current_participant) {
        current_participant <- row$PID
        count <- count + 1
        if(count <= 10) {  # Show first 10 participants
          cat(sprintf("\n%s (%s, %s):\n", 
                      ifelse(is.na(row$PID), "Unknown", row$PID), 
                      ifelse(is.na(row$study_completion_status), "Unknown", row$study_completion_status),
                      ifelse(is.na(row$Condition), "Unknown", row$Condition)))
        }
      }
      if(count <= 10 && !is.na(row$PID)) {
        cat(sprintf("  %d. %s (%d sessions, %.1f min, %d days, %.1f%%)\n", 
                    row$app_rank, 
                    ifelse(is.na(row$App), "Unknown", row$App), 
                    row$total_sessions, row$total_duration, 
                    row$days_used, row$pct_of_total_gaming))
      }
    }
  } else {
    cat("No valid gaming data found for participants.\n")
  }
  } else {
    cat("No gaming data available.\n")
  }
} else {
  cat("No ios_processed data available for gaming apps analysis.\n")
}
```
...
# Only proceed if we have availability_summary data
if (exists("availability_summary") && nrow(availability_summary) > 0 && exists("ios_processed") && nrow(ios_processed) > 0) {
  ...
  cat("No availability summary data available for trends visualization.\n")
}
```
...
if (exists("complete_grid") && nrow(complete_grid) > 0) {
  ...
  participant_patterns <- complete_grid %>%
    ...
    summarise(
      total_days_with_data = sum(has_data, na.rm = TRUE),
...
datatable(participant_patterns, 
  ...
  cat("No complete_grid data available for individual participant patterns analysis.\n")
  # Create empty objects to prevent errors in later sections
  participant_patterns <- data.frame()
}
```
...
if (exists("ios_processed") && nrow(ios_processed) > 0 && 
    exists("availability_summary") && nrow(availability_summary) > 0 && 
    exists("participant_patterns") && nrow(participant_patterns) > 0) {
...
if(exists("gaming_apps_summary") && nrow(gaming_apps_summary) > 0) {
  cat("- Gaming apps used by ≥2 participants:", nrow(gaming_apps_summary), "\n\n")
} else {
  cat("- Gaming apps used by ≥2 participants: Data not available\n\n")
}

cat("Condition Comparison:\n")
if(exists("pattern_summary") && nrow(pattern_summary) > 0) {
  ...
  cat("- Condition comparison data not available\n")
}
...
# Export processed data for further analysis (only if objects exist)
exported_files <- character(0)

if (exists("complete_grid") && nrow(complete_grid) > 0) {
  write_csv(complete_grid, "../../.tmp/ios/study_day_availability_matrix.csv")
  exported_files <- c(exported_files, "- study_day_availability_matrix.csv: Complete participant x study day matrix")
}

if (exists("availability_summary") && nrow(availability_summary) > 0) {
  write_csv(availability_summary, "../../.tmp/ios/daily_availability_summary.csv")
  exported_files <- c(exported_files, "- daily_availability_summary.csv: Daily availability percentages")
}

if (exists("participant_patterns") && nrow(participant_patterns) > 0) {
  write_csv(participant_patterns, "../../.tmp/ios/participant_data_patterns.csv")
  exported_files <- c(exported_files, "- participant_data_patterns.csv: Individual participant patterns")
}

if (exists("gaming_timeseries") && nrow(gaming_timeseries) > 0) {
  write_csv(gaming_timeseries, "../../.tmp/ios/gaming_timeseries_data.csv")
  exported_files <- c(exported_files, "- gaming_timeseries_data.csv: Gaming duration time series data")
}

if (exists("gaming_apps_summary") && nrow(gaming_apps_summary) > 0) {
  write_csv(gaming_apps_summary, "../../.tmp/ios/gaming_apps_summary.csv")
  exported_files <- c(exported_files, "- gaming_apps_summary.csv: Gaming apps usage statistics")
}

if (exists("top_gaming_by_participant") && nrow(top_gaming_by_participant) > 0) {
  write_csv(top_gaming_by_participant, "../../.tmp/ios/top_gaming_apps_by_participant.csv")
  exported_files <- c(exported_files, "- top_gaming_apps_by_participant.csv: Top gaming apps per participant")
}
...
if (length(exported_files) > 0) {
  ...
  cat("No data available for export.\n")
}
```