...
# Define the output file to check for caching
...
if (REFRESH_DATA && should_refresh_data(FINAL_OUTPUT_FILE, CACHE_DURATION_HOURS)) {
  cat("=== REFRESHING iOS DATA FROM QUALTRICS ===\n")
  cat("Running complete iOS data pipeline...\n\n")
  
  # Step 1: Download Qualtrics images
  cat("Step 1/5: Downloading Qualtrics survey responses and images...\n")
  ...
  cat("\nStep 2/5: Analyzing screenshots with Gemini OCR...\n")
  ...
  cat("\nStep 3/5: Aggregating participant data...\n")
  ...
  cat("\nStep 4/5: Converting summary to CSV...\n")
  ...
  cat("\nStep 5/5: Classifying gaming applications...\n")
...
# Check if final output file exists and show its info
if (file.exists(FINAL_OUTPUT_FILE)) {
  file_info <- file.info(FINAL_OUTPUT_FILE)
  cat(sprintf("✓ %s exists (modified: %s, size: %s bytes)\n", 
              basename(FINAL_OUTPUT_FILE), 
              file_info$mtime, 
...
# Load enrollment data from the daily survey file
...
cat("Number of participants with enrollment dates:", nrow(enrollment_data), "\n")
```
...
# Summary by participant with device type information
participant_summary <- ios_processed %>%
  group_by(PID, Condition, EnrollmentDate) %>%
  summarise(
...
datatable(participant_summary %>% select(-status_category), 
      caption = "iOS Participant Summary with Study Completion Status and Device Types",
      colnames = c("Participant ID", "Condition", "Enrollment Date", "Device Types", "Total Apps", "Total Sessions", 
