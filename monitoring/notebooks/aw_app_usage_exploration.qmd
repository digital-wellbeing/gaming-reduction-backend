...
# Use platform-specific participant report if available, otherwise fall back to generic
PARTICIPANT_REPORT_ANDROID_FILE <- "../../.tmp/participant_report_android.csv"
if (file.exists(PARTICIPANT_REPORT_ANDROID_FILE)) {
  PARTICIPANT_REPORT_FILE <- PARTICIPANT_REPORT_ANDROID_FILE
  cat("Using Android-specific participant report\n")
} else {
  cat("Using generic participant report (backward compatibility)\n")
}
...
if (REFRESH_DATA && (should_refresh_data(AW_DATA_FILE, CACHE_DURATION_HOURS) || should_refresh_data(PARTICIPANT_REPORT_FILE, CACHE_DURATION_HOURS))) {
  ...
  # Get absolute paths
  script_path <- normalizePath("../../monitoring/join_diary_activitywatch.py")
  output_dir <- normalizePath("../../.tmp")
  
  step1_result <- tryCatch({
    cat("⏱️ This may take several minutes to pull data from Supabase and Qualtrics...\n")
    system2(
      "python3",
      args = c(script_path, "--output-dir", output_dir, "--diary-file", file.path(output_dir, "diary_responses_lifetime.csv"), "--verbose"),
      stdout = TRUE,
      stderr = TRUE,
      wait = TRUE,
  ...
  if (!is.null(attr(step1_result, "status")) && attr(step1_result, "status") != 0) {
    cat("⚠️ Warning: Step 1 exited with non-zero status:", attr(step1_result, "status"), "\n")
    ...
    cat("❌ Step 1 failed or timed out\n")
  }
...
if (REFRESH_DATA && should_refresh_data(AW_ENRICHED_FILE, CACHE_DURATION_HOURS)) {
  ...
  # Get absolute paths
  classifier_script <- normalizePath("../../monitoring/ocr/app_game_classifier.py")
  aw_data_path <- normalizePath(AW_DATA_FILE)
  aw_enriched_path <- normalizePath(AW_ENRICHED_FILE, mustWork = FALSE)
  
  step2_result <- tryCatch({
    cat("⏱️ This may take several minutes to classify gaming apps...\n")
    system2(
      "python3",
      args = c(classifier_script, aw_data_path, "--format", "activitywatch", "--output", aw_enriched_path),
      stdout = TRUE,
      stderr = TRUE,
      wait = TRUE,
  ...
  if (!is.null(attr(step2_result, "status")) && attr(step2_result, "status") != 0) {
    cat("⚠️ Warning: Step 2 exited with non-zero status:", attr(step2_result, "status"), "\n")
    ...
    cat("❌ Step 2 failed or timed out\n")
  }
  
  cat("\n🎉 ActivityWatch data pipeline completed!\n\n")
  ...
  aw_data_path <- normalizePath(AW_DATA_FILE)
  aw_enriched_path <- normalizePath(AW_ENRICHED_FILE, mustWork = FALSE)
  system2("python3", args = c(classifier_script, aw_data_path, "--format", "activitywatch", "--output", aw_enriched_path))
  cat("✅ Game classification completed\n\n")
}
...
if (nrow(participants_missing_condition) > 0) {
  cat("⚠️ WARNING: Found", nrow(participants_missing_condition), "participants without Condition data:\n")
...
cat("- Participants with data for ≥20 days:", participants_with_full_study, "out of", total_aw_participants, "\n")
